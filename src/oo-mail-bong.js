(function () {
    ...
    function processFolders(folders) {
        ...
        const unread = Number(
            inbox.unread_messages !== undefined
    ...
    function hookMailFolders() {
        ...
        if (
            !window.Teamlab ||
            typeof window.Teamlab.getMailFolders !== "function"
        ) {
        ...
        window.Teamlab.getMailFolders = function (params, options) {
            ...
            const wrappedOptions = Object.assign({}, options, {
                success: function () {
                    try {
                        processFolders(arguments[1]);
                    } catch (err) {
                        console.error("[OO Mail Alert]", err);
                    }
